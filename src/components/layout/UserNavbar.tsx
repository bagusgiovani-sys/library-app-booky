...
export default function UserNavbar() {
  ...
  const menuItems = [
    { icon: <User size={15} />, label: "Profile", route: ROUTES.PROFILE },
    {
      ...
      label: "Borrowed",
      route: ROUTES.PROFILE_BORROWED,
    },
    {
      icon: <BookOpen size={15} />,
      label: "Reviews",
      route: ROUTES.PROFILE_REVIEWS,
    },
  ];

  return (
    <nav className="sticky top-0 z-50 w-full bg-[#f6f9fe] shadow-sm">
      <Container className="py-5 mb-5">
        {/* Main row */}
        <div className="flex items-center justify-between">
          <Link to={ROUTES.HOME} className="flex items-center gap-2">
            <img src={logo} alt="Booky" className="w-8 h-8" />
            <span className="hidden sm:block text-lg font-bold text-[#1c65da]">
              Booky
            </span>
          </Link>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <Avatar className="w-8 h-8">
                  ...
                  <AvatarFallback className="bg-[var(--color-primary-200)] text-[var(--color-primary-300)]">
                ...
                {open && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors text-left text-[var(--color-accent-red)]"
                    >
                      <LogOut size={15} />
                      Logout
                    </button>
        ...
        <AnimatePresence>
          {searchOpen && (
            ...
            >
              <div className="flex items-center gap-2 bg-white rounded-full px-4 py-2.5 border border-gray-200 mt-3">
                ...
                <input
                  ...
                  className="flex-1 text-sm bg-transparent outline-none border-none text-gray-700 placeholder:text-gray-400 font-[inherit]"
                />
              </div>
            </motion.div>
          )}
